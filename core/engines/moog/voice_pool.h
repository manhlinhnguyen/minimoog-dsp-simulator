// ─────────────────────────────────────────────────────────
// FILE: core/engines/moog/voice_pool.h
...
private:
    ...
    int       notePriority_  = 0;     // 0=last, 1=lowest, 2=highest
    int       maxVoices_    = 1;
    float     unisonDetune_ = 0.0f;  // total spread in cents
    float     outputGain_   = 1.0f;  // 1/sqrt(N) for headroom
    uint32_t  unisonSeed_   = 31337u; // LCG seed for analog random detune
    ...
    const HeldNote* topHeldByPriority() const     noexcept;
};
