...
// ── Tab: Sequencer ───────────────────────────────────────
static void renderSeq(AtomicParamStore& p, EngineManager& mgr,
                      PatternStorage& patterns) {
    if (!s_seqInitialized) {
        for (int i = 0; i < StepSequencer::MAX_STEPS; ++i)
            s_steps[i] = mgr.getSeqStep(i);
    ...
    SeqDisplay::draw(s_steps.data(), steps, mgr.getSeqCurrentStep(), p);
...
void render(AtomicParamStore& params, EngineManager& mgr,
    ...
    // ── Global BPM (shared by Arp, Seq, and Delay BpmSync) ──
    float bpm = params.get(P_BPM);
    ImGui::SetNextItemWidth(160.f);
    if (ImGui::SliderFloat("BPM", &bpm,
            PARAM_META[P_BPM].minVal, PARAM_META[P_BPM].maxVal, "%.0f"))
        params.set(P_BPM, bpm);
    ImGui::Separator();

    // Track which tab is active; tab selection drives enable state.
    static int s_activeTab     = 0;  // 0=Keyboard,1=MIDI,2=Arp,3=Chord,4=Scale,5=Seq
    ...
    if (ImGui::BeginTabBar("MusicTabs")) {
        ...
        if (ImGui::BeginTabItem("MIDI Player")) {
            s_newTab = 1;
            PanelMidiPlayer::renderTab(midiSt, midiPlayer);
            ImGui::EndTabItem();
        }
    ...
    params.set(P_ARP_ON,   s_activeTab == 2 ? 1.f : 0.f);
    params.set(P_CHORD_ON, s_activeTab == 3 ? 1.f : 0.f);
    params.set(P_SCALE_ON, s_activeTab == 4 ? 1.f : 0.f);
    params.set(P_SEQ_ON,   s_activeTab == 5 ? 1.f : 0.f);
